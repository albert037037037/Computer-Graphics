...
// AS3
bool magLinear = false, minLinear = false;
GLuint iLoctextureFromMain;
int thisIsEye;
GLuint iLocthisIsEye;
float xshift[7] = {0.0, 0.0, 0.0, 0.0, 0.5, 0.5, 0.5};
float yshift[7] = {0.0, 0.75, 0.5, 0.25, 0.0, 0.75, 0.5};
int *xshift_idx, *yshift_idx;
GLuint iLocxshift, iLocyshift;
int modelNum = 0;
...
void RenderScene(int per_vertex_or_per_pixel) {	
	...
	for (int i = 0; i < models[cur_idx].shapes.size(); i++) 
	{
        ...
        glUniform1i(iLocthisIsEye, thisIsEye);
        glUniform1f(iLocxshift, xshift[xshift_idx[cur_idx]]);
        glUniform1f(iLocyshift, yshift[yshift_idx[cur_idx]]);
...
void KeyCallback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	if (action == GLFW_PRESS) {
        ...
        case GLFW_KEY_B:
            ...
            break;
        case GLFW_KEY_RIGHT:
            xshift_idx[cur_idx] = (xshift_idx[cur_idx] != 6) ? xshift_idx[cur_idx] + 1 : 0;
            yshift_idx[cur_idx] = (yshift_idx[cur_idx] != 6) ? yshift_idx[cur_idx] + 1 : 0;
            break;
        case GLFW_KEY_LEFT:
            xshift_idx[cur_idx] = (xshift_idx[cur_idx] == 0) ? 6 : xshift_idx[cur_idx] - 1;
            yshift_idx[cur_idx] = (yshift_idx[cur_idx] == 0) ? 6 : yshift_idx[cur_idx] - 1;
            break;
		default:
			break;
		}
		...
		if (starting_press_x < 0 || starting_press_y < 0) {
			...
			switch (cur_trans_mode)
			{
                ...
                case GeoRotation:
                    ...
                    break;
                case Lighting:
                    lightInfo[lightIndex].position.x += 0.005 * diff_x;
                    lightInfo[lightIndex].position.y -= 0.005 * diff_y;
                    break;
            }
		}
	}
...
void setUniformVariables()
{
    ...
    iLoctextureFromMain = glGetUniformLocation(program, "textureFromMain");
    iLocthisIsEye = glGetUniformLocation(program, "thisIsEye");
    
    // AS2
    is_perpixel = glGetUniformLocation(program, "is_perpixel");
    iLocviewMatrix = glGetUniformLocation(program, "viewMatrix");
    iLoclightIndex = glGetUniformLocation(program, "lightIndex");
    iLocshiness = glGetUniformLocation(program, "shiness");
    iLocxshift = glGetUniformLocation(program, "xshift");
    iLocyshift = glGetUniformLocation(program, "yshift");
    ...
    iLoclightInfo[0].position = glGetUniformLocation(program, "lightInfo[0].position");
    iLoclightInfo[0].spotDir = glGetUniformLocation(program, "lightInfo[0].spotDir");
    iLoclightInfo[0].spotExponent = glGetUniformLocation(program, "lightInfo[0].spotExponent");
    iLoclightInfo[0].spotCutoff = glGetUniformLocation(program, "lightInfo[0].spotCutoff");
    iLoclightInfo[0].diffuse = glGetUniformLocation(program, "lightInfo[0].diffuse");
    iLoclightInfo[0].Ambient = glGetUniformLocation(program, "lightInfo[0].Ambient");
    iLoclightInfo[0].constantAttenuation = glGetUniformLocation(program, "lightInfo[0].constantAttenuation");
    iLoclightInfo[0].linearAttenuation = glGetUniformLocation(program, "lightInfo[0].linearAttenuation");
    iLoclightInfo[0].quadraticAttenuation = glGetUniformLocation(program, "lightInfo[0].quadraticAttenuation");
    iLoclightInfo[0].specular = glGetUniformLocation(program, "lightInfo[0].specular");
    
    iLoclightInfo[1].position = glGetUniformLocation(program, "lightInfo[1].position");
    iLoclightInfo[1].spotDir = glGetUniformLocation(program, "lightInfo[1].spotDir");
    iLoclightInfo[1].spotExponent = glGetUniformLocation(program, "lightInfo[1].spotExponent");
    iLoclightInfo[1].spotCutoff = glGetUniformLocation(program, "lightInfo[1].spotCutoff");
    iLoclightInfo[1].diffuse = glGetUniformLocation(program, "lightInfo[1].diffuse");
    iLoclightInfo[1].Ambient = glGetUniformLocation(program, "lightInfo[1].Ambient");
    iLoclightInfo[1].constantAttenuation = glGetUniformLocation(program, "lightInfo[1].constantAttenuation");
    iLoclightInfo[1].linearAttenuation = glGetUniformLocation(program, "lightInfo[1].linearAttenuation");
    iLoclightInfo[1].quadraticAttenuation = glGetUniformLocation(program, "lightInfo[1].quadraticAttenuation");
    iLoclightInfo[1].specular = glGetUniformLocation(program, "lightInfo[1].specular");
    
    iLoclightInfo[2].position = glGetUniformLocation(program, "lightInfo[2].position");
    iLoclightInfo[2].spotDir = glGetUniformLocation(program, "lightInfo[2].spotDir");
    iLoclightInfo[2].spotExponent = glGetUniformLocation(program, "lightInfo[2].spotExponent");
    iLoclightInfo[2].spotCutoff = glGetUniformLocation(program, "lightInfo[2].spotCutoff");
    iLoclightInfo[2].diffuse = glGetUniformLocation(program, "lightInfo[2].diffuse");
    iLoclightInfo[2].Ambient = glGetUniformLocation(program, "lightInfo[2].Ambient");
    iLoclightInfo[2].constantAttenuation = glGetUniformLocation(program, "lightInfo[2].constantAttenuation");
    iLoclightInfo[2].linearAttenuation = glGetUniformLocation(program, "lightInfo[2].linearAttenuation");
    iLoclightInfo[2].quadraticAttenuation = glGetUniformLocation(program, "lightInfo[2].quadraticAttenuation");
    iLoclightInfo[2].specular = glGetUniformLocation(program, "lightInfo[2].specular");
...
void setupRC()
{
    ...
    xshift_idx = (int*) malloc(sizeof(int) * modelNum);
    yshift_idx = (int*) malloc(sizeof(int) * modelNum);
...
int main(int argc, char **argv)
{
    ...
    // create window
	GLFWwindow* window = glfwCreateWindow(WINDOW_WIDTH, WINDOW_HEIGHT, "107062321 HW3", NULL, NULL);
    if (window == NULL)
    {
